...
export class GetImageBySpotNameUseCase {
  ...
  async execute(spotName: string): Promise<GetImageListDTO[] | null> {
    ...
    if (!spots || spots.length === 0) {
      throw new Error("No spots found for the given name");
    }

    const images: GetImageListDTO[] = [];

    // 2. 해당 명소들에 작성된 팁들을 조회
    for (const spot of spots) {
      const tips = await this.tipRepository.getTipsBySpotId(
        spot.id,
        "createdAt"
      );
      ...
      for (const tip of tips) {
        ...
        for (const image of imagesForTip) {
          if (image.path) {
            // 작성자 정보 조회
            const user: GetUserDto | null =
              await this.userRepository.getUserById(tip.userId);
            if (!user) continue;
